...
export function LotDetail({ lot, onBack, onValidate, account }: LotDetailProps) {
  ...
  const canValidateStep = (stepIndex: number) => {
    ...
    if (step.status === 1) return false; // Already completed
    if (stepIndex > 0 && lot.steps[stepIndex - 1].status !== 1) return false; // Previous step not completed
    if (step.validators.length === 0) return true; // Open to all
    if (!account) return false; // Not connected
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              {isFullyCompleted ? (
                ...
                <span className="text-xs font-medium text-stone-700 bg-stone-50 px-2 py-0.5 rounded">
                  En cours
                </span>
              )}
            </div>
